...
static void lcd_prepare_laser_menu()
{
	...
	MENU_ITEM(gcode, MSG_PARK, PSTR("G31"));
...
static void lcd_modesel_menu()
{
    ...
    MENU_ITEM(function, MSG_MODE_CNC, mode_cnc);
    MENU_ITEM(function, MSG_MODE_LASERCUT, mode_lasercut);
...
static void lcd_move_x()
{
    if (encoderPosition != 0)
    {
        ...
        plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 0, manual_feedrate[X_AXIS]/60, 0, LaserPWR);
...
static void lcd_move_y()
{
    if (encoderPosition != 0)
    {
        ...
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], 0, manual_feedrate[Y_AXIS]/60, 0, LaserPWR);
        #else
        plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], manual_feedrate[Y_AXIS]/60, 0, LaserPWR);
        #endif
        lcdDrawUpdate = 1;
    }
    if (lcdDrawUpdate)
...
static void lcd_move_e()
{
    if (encoderPosition != 0)
    {
        ...
        #ifdef DELTA
        calculate_delta(current_position);
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[E_AXIS]/60, 0, LaserPWR);
        #else
        plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], manual_feedrate[E_AXIS]/60, 0, LaserPWR);
        #endif
        lcdDrawUpdate = 1;
    }
    if (lcdDrawUpdate)
...
{
    START_MENU();
    MENU_ITEM(back, MSG_PREPARE, lcd_prepare_laser_menu);
...
static void lcd_laser_menu()
{
    ...
    MENU_ITEM(back, MSG_MAIN, lcd_main_menu);
	if(!LaserEnabled)
		MENU_ITEM(gcode, MSG_MENU_LASER_PWR_ON, PSTR("M10 S0"));
	else
		MENU_ITEM(gcode, MSG_MENU_LASER_PWR_OFF, PSTR("M11"));
	if(!LaserState)
		MENU_ITEM(gcode, MSG_LASEREN, PSTR("M3 S0"));
	else
		MENU_ITEM(gcode, MSG_LASERDIS, PSTR("M5"));
	//MENU_ITEM_EDIT(float3, MSG_MENU_LASER_FOCUS, &laserfocus, 0, 60);
	MENU_ITEM(submenu, "Focus", lcd_move_focus);
	END_MENU();
}



static void lcd_zero_offset()
{
    laserfocus = 0;
    lcd_move_focus();
}

static void lcd_move_menu_Z_1mm()
{
    move_menu_scale = 1.0;
    lcd_move_z_focus();
}
static void lcd_move_menu_Z_01mm()
{
    move_menu_scale = 0.1;
    lcd_move_z_focus();
}

static void lcd_move_menu_axisZ()
{
    START_MENU();
    MENU_ITEM(back, MSG_MOVE_AXIS, lcd_move_focus);
	MENU_ITEM(submenu, "Move 1mm", lcd_move_menu_Z_1mm);
    MENU_ITEM(submenu, "Move 0.1mm", lcd_move_menu_Z_01mm);
    END_MENU();
}

/* static void lcd_set_focus_offset()
{
	plan_set_z_position(0);
	st_synchronize();
	lcd_quick_feedback();
    currentMenu = lcd_move_focus;
    encoderPosition = 0;
} */
...
static void lcd_move_z_focus()
{
    if (encoderPosition != 0)
    {
        ...
        lcdDrawUpdate = 1;
		laserfocus = current_position[Z_AXIS];
    }
    if (lcdDrawUpdate)
    {
        lcd_implementation_drawedit(PSTR("Z"), ftostr31(current_position[Z_AXIS]));
    ...
    {
        lcd_quick_feedback();
        currentMenu = lcd_move_menu_axisZ;
        encoderPosition = 0;
    }
}
